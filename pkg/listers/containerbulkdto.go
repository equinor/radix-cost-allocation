...
type containerDtoMapperFunc func(*corev1.Pod, map[string]*v1.RadixRegistration, map[string]*corev1.LimitRange) []repository.ContainerBulkDto
...
func NewContainerBulkDtoLister(podLister PodLister,
	rrLister RadixRegistrationLister,
	limitRangeLister LimitRangeLister) ContainerBulkDtoLister {
...
func (b *containerBulkDtoLister) List() []repository.ContainerBulkDto {
	return buildContainerDtoFromPodList(b.podLister.List(), b.rrLister.List(), b.limitRangeLister.List(), b.mapper)
}

func buildContainerDtoFromPodList(pods []*corev1.Pod, rrList []*v1.RadixRegistration, limitRanges []*corev1.LimitRange, mapper containerDtoMapperFunc) []repository.ContainerBulkDto {
	...
	for _, pod := range pods {
		if pod != nil {
			podContainers := mapper(pod, rrMap, limitRangeMap)
